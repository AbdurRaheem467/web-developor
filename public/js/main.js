...
class PortfolioApp {
  ...
  /**
   * Render Technical Skills Section in Continuous Mini-Slides Marquee format
   * (Choti Choti Slides Jo Mussassal Chalti Rahein)
   */
  static renderSkills() {
    ...
    const categoryThemeMap = {
      "Frontend Core": {
        ...
        iconBg: "bg-cyan-500/10 border-cyan-500/30 text-cyan-400 group-hover:bg-cyan-500 group-hover:text-black group-hover:shadow-[0_0_20px_rgba(6,182,212,0.7)]",
        accentText: "text-cyan-400"
      },
      "Frameworks": {
        badgeBg: "bg-indigo-950/80 text-indigo-300 border-indigo-800/60",
        iconBg: "bg-indigo-500/10 border-indigo-500/30 text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white group-hover:shadow-[0_0_20px_rgba(99,102,241,0.7)]",
        accentText: "text-indigo-400"
      },
      "Workflow": {
        badgeBg: "bg-amber-950/80 text-amber-300 border-amber-800/60",
        iconBg: "bg-amber-500/10 border-amber-500/30 text-amber-400 group-hover:bg-amber-500 group-hover:text-black group-hover:shadow-[0_0_20px_rgba(245,158,11,0.7)]",
        accentText: "text-amber-400"
      },
      "Design & UX": {
        badgeBg: "bg-emerald-950/80 text-emerald-300 border-emerald-800/60",
        iconBg: "bg-emerald-500/10 border-emerald-500/30 text-emerald-400 group-hover:bg-emerald-500 group-hover:text-black group-hover:shadow-[0_0_20px_rgba(16,185,129,0.7)]",
        accentText: "text-emerald-400"
      },
      "Engineering": {
        badgeBg: "bg-purple-950/80 text-purple-300 border-purple-800/60",
        iconBg: "bg-purple-500/10 border-purple-500/30 text-purple-400 group-hover:bg-purple-500 group-hover:text-white group-hover:shadow-[0_0_20px_rgba(168,85,247,0.7)]",
        accentText: "text-purple-400"
      }
    };

    // Track 1 Skills: Core Web & Modern Frameworks
    const streamSkillsRow1 = [
      { name: "HTML5", category: "Frontend Core", level: "Advanced", pct: 95, icon: "code-2" },
      { name: "CSS3", category: "Frontend Core", level: "Advanced", pct: 92, icon: "palette" },
      { name: "JavaScript", category: "Frontend Core", level: "Advanced", pct: 90, icon: "file-code" },
      { name: "React", category: "Frameworks", level: "Proficient", pct: 88, icon: "atom" },
      { name: "Next.js", category: "Frameworks", level: "Proficient", pct: 85, icon: "layers" },
      { name: "Tailwind CSS", category: "Frameworks", level: "Advanced", pct: 94, icon: "sparkles" },
      { name: "TypeScript", category: "Frameworks", level: "Proficient", pct: 84, icon: "terminal" },
      { name: "Vite & Tooling", category: "Workflow", level: "Advanced", pct: 90, icon: "zap" },
    ];

    // Track 2 Skills: Workflow, UX, APIs & Performance
    const streamSkillsRow2 = [
      { name: "Git & GitHub", category: "Workflow", level: "Proficient", pct: 86, icon: "git-branch" },
      { name: "Responsive Design", category: "Design & UX", level: "Expert", pct: 98, icon: "smartphone" },
      { name: "UI/UX Design", category: "Design & UX", level: "Proficient", pct: 88, icon: "layout-template" },
      { name: "RESTful APIs", category: "Workflow", level: "Proficient", pct: 87, icon: "webhook" },
      { name: "Web Performance", category: "Engineering", level: "Advanced", pct: 92, icon: "gauge" },
      { name: "Clean Architecture", category: "Engineering", level: "Advanced", pct: 90, icon: "shield-check" },
      { name: "SEO & Core Vitals", category: "Workflow", level: "Proficient", pct: 89, icon: "search" },
      { name: "Cross-Browser UI", category: "Design & UX", level: "Expert", pct: 95, icon: "monitor" },
    ];

    const generateCardsHtml = (items) => items.map(skill => {
      const theme = categoryThemeMap[skill.category] || categoryThemeMap["Frontend Core"];
      return `
        <div class="skill-mini-card group">
          <div class="skill-mini-icon ${theme.iconBg} border">
            <i data-lucide="${skill.icon}" class="w-5 h-5"></i>
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center justify-between gap-1">
              <h3 class="text-sm font-bold text-white group-hover:${theme.accentText} transition-colors truncate">
                ${skill.name}
              </h3>
              <span class="text-[10px] font-mono-code font-bold ${theme.accentText}">${skill.pct}%</span>
            </div>
            <div class="flex items-center gap-1.5 mt-0.5">
              <span class="text-[10px] font-mono-code text-slate-400 truncate">${skill.category}</span>
              <span class="text-slate-600 text-[10px]">&bull;</span>
              <span class="text-[10px] font-mono-code text-slate-500">${skill.level}</span>
            </div>
          </div>
        </div>
      `;
    ...
    // Populate Row 1 (duplicated for mathematically seamless infinite scroll)
    const cardsRow1Html = generateCardsHtml(streamSkillsRow1);
    row1.innerHTML = `
      <div class="marquee-content">${cardsRow1Html}</div>
      <div class="marquee-content" aria-hidden="true">${cardsRow1Html}</div>
    `;

    // Populate Row 2 (duplicated for mathematically seamless infinite scroll)
    const cardsRow2Html = generateCardsHtml(streamSkillsRow2);
    row2.innerHTML = `
      <div class="marquee-content">${cardsRow2Html}</div>
      <div class="marquee-content" aria-hidden="true">${cardsRow2Html}</div>
    `;

    // Populate Grid View (All skills combined)
    if (gridTrack) {
      const allUniqueSkills = [...streamSkillsRow1, ...streamSkillsRow2];
      gridTrack.innerHTML = allUniqueSkills.map(skill => {
        const theme = categoryThemeMap[skill.category] || categoryThemeMap["Frontend Core"];
        return `
          <div class="glass-card p-4 rounded-xl border border-slate-800 flex items-center gap-3.5 group hover:border-cyan-500/40 transition-all duration-300">
            <div class="w-10 h-10 rounded-lg ${theme.iconBg} border flex items-center justify-center shrink-0">
              <i data-lucide="${skill.icon}" class="w-5 h-5"></i>
            </div>
            <div class="min-w-0 flex-1">
              <h4 class="text-xs font-bold text-white group-hover:${theme.accentText} transition-colors truncate">${skill.name}</h4>
              <p class="text-[10px] font-mono-code text-slate-400 mt-0.5">${skill.category} &bull; ${skill.pct}%</p>
            </div>
          </div>
        `;
      }).join("");
  ...
  /**
   * Set up Continuous Mini-Slides Marquee Stream (Choti Choti Slides Jo Mussassal Chalti Rahein)
   */
  static setupSkillsSlider() {
    const wrapper = document.getElementById("skillsMarqueeWrapper");
    if (!wrapper) return;

    window.skillsMarquee = new SkillsContinuousMarqueeController();
  }
...
/**
 * CONTINUOUS MINI-SLIDES MARQUEE CONTROLLER
 * ==============================================================================
 * Controls continuous non-stop stream of tech slides:
 * Pause/Play, Speed adjustment (0.7x, 1.0x, 1.5x), and Stream/Grid mode toggling.
 * ==============================================================================
 */
class SkillsContinuousMarqueeController {
  constructor() {
    this.wrapper = document.getElementById("skillsMarqueeWrapper");
    this.streamWrapper = document.getElementById("skillsStreamWrapper");
    this.gridWrapper = document.getElementById("skillsGridWrapper");
    this.pauseBtn = document.getElementById("skillsMarqueePauseBtn");
    this.playIcon = document.getElementById("skillsPlayIcon");
    this.pauseText = document.getElementById("skillsPauseBtnText");
    this.statusPulse = document.getElementById("skillsAutoplayPulse");
    this.statusText = document.getElementById("skillsAutoplayStatus");
    this.slowBtn = document.getElementById("speedSlowBtn");
    this.normalBtn = document.getElementById("speedNormalBtn");
    this.fastBtn = document.getElementById("speedFastBtn");
    this.streamModeBtn = document.getElementById("modeStreamBtn");
    this.gridModeBtn = document.getElementById("modeGridBtn");
  ...
  togglePause() {
    ...
    if (this.playIcon) {
      this.playIcon.setAttribute("data-lucide", this.isPaused ? "play" : "pause");
      PortfolioApp.refreshIcons();
    }
  }
  ...
  setSpeed(speed) {
    ...
    this.wrapper.classList.remove("speed-slow", "speed-normal", "speed-fast");
    this.wrapper.classList.add(`speed-${speed}`);

    const buttons = [
      { el: this.slowBtn, key: "slow" },
      { el: this.normalBtn, key: "normal" },
      { el: this.fastBtn, key: "fast" },
    ];

    buttons.forEach(b => {
      if (!b.el) return;
      if (b.key === speed) {
        b.el.className = "px-2.5 py-0.5 rounded-full text-[11px] font-mono-code bg-indigo-600 text-white font-bold transition-all shadow-sm";
      } else {
        b.el.className = "px-2.5 py-0.5 rounded-full text-[11px] font-mono-code text-slate-400 hover:text-white transition-all";
      }
    });
  }
  ...
  setGridMode(isGrid) {
    ...
    if (this.streamWrapper && this.gridWrapper) {
      if (isGrid) {
        this.streamWrapper.classList.add("hidden");
        this.gridWrapper.classList.remove("hidden");
        if (this.streamModeBtn) this.streamModeBtn.className = "px-3 py-1 rounded-full text-xs font-semibold text-slate-400 hover:text-white transition-all flex items-center gap-1.5";
        if (this.gridModeBtn) this.gridModeBtn.className = "px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-indigo-500 to-cyan-500 text-white transition-all shadow-sm flex items-center gap-1.5";
      } else {
        this.streamWrapper.classList.remove("hidden");
        this.gridWrapper.classList.add("hidden");
        if (this.streamModeBtn) this.streamModeBtn.className = "px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-indigo-500 to-cyan-500 text-white transition-all shadow-sm flex items-center gap-1.5";
        if (this.gridModeBtn) this.gridModeBtn.className = "px-3 py-1 rounded-full text-xs font-semibold text-slate-400 hover:text-white transition-all flex items-center gap-1.5";
      }
    }
  }
  ...
  bindEvents() {
    ...
    this.slowBtn?.addEventListener("click", () => this.setSpeed("slow"));
    this.normalBtn?.addEventListener("click", () => this.setSpeed("normal"));
    this.fastBtn?.addEventListener("click", () => this.setSpeed("fast"));

    this.streamModeBtn?.addEventListener("click", () => this.setGridMode(false));
    this.gridModeBtn?.addEventListener("click", () => this.setGridMode(true));
  }
}
